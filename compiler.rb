...
def handle_shaders(use_glsl_min, glsl_min_rewrite_all, shared, mdx, m3, srcpath, output)
  ...
  if use_glsl_min
    ...
    File.open(srcpath + output, "w") { |out|
      ...
      if glsl_min_rewrite_all
        out.write("\n\nvar PARAMETERMAP = {\n\t");
        out.write(minified[1].to_a().collect { |v| "\"#{v[0]}\":\"#{v[1]}\"" }.join(",\n\t"))
        out.write("\n};\n\nvar MEMBERMAP = {\n\t");
        out.write(minified[2].to_a().collect { |v| "\"#{v[0]}\":\"#{v[1]}\"" }.join(",\n\t"))
        out.write("\n};")
      else
...
handle_shaders(USE_GLSL_MIN, GLSL_MIN_REWRITE_ALL, SHARED_SHADERS, MDX_SHADERS, M3_SHADERS, "src/viewer/", "shaders.js")
