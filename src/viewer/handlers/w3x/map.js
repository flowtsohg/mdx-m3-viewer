...
export default class W3xMap extends Resource {
  ...
  loadTerrain(environment) {
    ...
    let slk = this.fileCache.get('terrain');
  ...
  prepareTilePoints() {
    ...
    let centerOffset = this.offset;

    for (let y = 0; y < mapSize[1]; y++) {
      for (let x = 0; x < mapSize[0]; x++) {
        let tile = tilepoints[y][x];
        let ltMask = 0, rtMask = 0, lbMask = 0, rbMask = 0;
  ...
  loadWater() {
    ...
    for (let y = 0; y < mapSize[1] - 1; y++) {
      for (let x = 0; x < mapSize[0] - 1; x++) {
        let tile1 = tilepoints[y][x],
    ...
    this.viewer.loadTextureAtlas('mapWater', textures)
      .then((atlas) => {
        let texture = this.viewer.load(atlas.texture);

        let gl = this.viewer.gl;
        gl.bindTexture(gl.TEXTURE_2D, texture.webglResource);
        texture.setParameters(gl.CLAMP_TO_EDGE, gl.CLAMP_TO_EDGE, gl.LINEAR, gl.LINEAR);

        let model = this.viewer.load({
  ...
  loadTerrainGeometry() {
    ...
    let blightTextureIndex = this.blightTextureIndex;
    ...
    for (let y = 0; y < mapSize[1] - 1; y++) {
      for (let x = 0; x < mapSize[0] - 1; x++) {
        ...
        let textures = unique([texture1, texture2, texture3, texture4]);

        for (let texture = 0; texture < textures.length; texture++) {
          ...
          let extended = (tilesetTextures[t].width === 512);
          let width = extended ? 1 / 8 : 1 / 4;
          let height = 1 / 4;
          let offsetX = 0;
          let offsetY = 0;
          ...
          let pixelSizeX = 1 / tilesetTextures[t].width;
          let pixelSizeY = 1 / tilesetTextures[t].height;
          offsetX += pixelSizeX;
          offsetY += pixelSizeY;
          width -= 2 * pixelSizeX;
          height -= 2 * pixelSizeY;
    ...
    for (let i = 0, l = tilesetTextures.length; i < l; i++) {
      if (vertices[i]) {
  ...
  applyModificationTable(modificationTable, dataTable, metadataTable) {
    let modifications = modificationTable.objects;

    for (let i = 0, l = modifications.length; i < l; i++) {
  ...
  applyModificationObject(modification, dataTable, metadataTable) {
    let row = dataTable.getRow(modification.oldID);
    ...
    if (row) {
      ...
      let modifications = modification.modifications;
  ...
  applyModification(modification, row, metadataTable) {
    let metadata = metadataTable.map[modification.id];
