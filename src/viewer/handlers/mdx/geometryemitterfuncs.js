...
function bindParticleEmitter2Buffer(emitter, buffer) {
  ...
  let teamColor = emitter.instance.teamColor;
  let offset = 0;
...
/**
 * @param {EventObjectSplEmitter} emitter
 * @param {Shader} shader
 */
...
/**
 * @param {EventObjectUbrEmitter} emitter
 * @param {Shader} shader
 */
...
/**
 * @param {ParticleEmitter2|RibbonEmitter|EventObjectSplEmitter|EventObjectUbrEmitter} emitter
 * @param {Shader} shader
 */
export function renderEmitter(emitter, shader) {
  ...
  if (alive) {
    ...
    gl.extensions.instancedArrays.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 6, alive);
  }
}
