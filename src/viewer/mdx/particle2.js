...
Particle2.prototype = {
  ...
  update: function (emitter, sequence, frame, counter) {
    ...
    var lifeFactor = 1 - (this.health / emitter.lifespan);
    var scale;
    var tempFactor;
    ...
    if (interval < 3 && currentFrame >= emitter.framesPerInterval[interval]) {
      interval++;
      
      this.interval = interval;
    }
    
    var frameInterval = emitter.intervals[interval];
    var frameIntervalStart = emitter.intervalStarts[interval];
    var index = emitter.intervalLocalStarts[interval] + ((currentFrame - frameIntervalStart) % frameInterval);
