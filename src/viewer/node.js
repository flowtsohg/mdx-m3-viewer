...
let Node = (superclass) => class extends superclass {
    constructor() {
        super();
 ...
 */
export class SceneNode extends Node(Object) {};

/**
 * A scene node that is also an event dispatcher.
 */
export class EventNode extends Node(EventDispatcher) {};
...
export class SkeletalNode {
     ...
     * Unlike the SceneNode variant, this doesn't change any state in the parent node.
     * This is because skeletal nodes are controlled by the code that uses them.
     * 
     * @param {SceneNode|EventNode|SkeletalNode} parent 
     */
    setParent(parent) {
        this.parent = parent;
    }
    ...
    recalculateTransformation() {
        ...
        mat4.fromRotationTranslationScaleOrigin(localMatrix, localRotation, this.localLocation, computedScaling, pivot);
 ...
 * @returns {Object}
 */
export function createSkeletalNodes(count) {
